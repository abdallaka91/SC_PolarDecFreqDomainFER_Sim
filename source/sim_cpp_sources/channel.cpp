...
void PoAwN::channel::EncodeChanBPSK_BinCCSK(
    std::mt19937 &gen, decoder_parameters &dec_param, const table_GF &table,
    const float SNR, const vector<vector<uint16_t>> &bin_table,
    vector<decoder_t> &chan_LLR_sorted, vector<uint16_t> &KSYMB,
  ...
  for (int i = 0; i < K; i++)
    dec_param.ucap[dec_param.n][KSYMB_Idx[i]] = KSYMB[i];
  ...
  Encoder(table.ADDDEC, table.MULDEC, dec_param.polar_coeff, dec_param.ucap,
          NSYMB);
  // for (int y = 0; y < N; y++)
  ...
  vector<vector<softdata_t>> noisy_sig(
      N, vector<softdata_t>(bin_table[0].size(), (softdata_t)0.0));
