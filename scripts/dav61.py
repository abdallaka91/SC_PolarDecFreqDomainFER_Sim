...
snr_values = np.arange(-16.5, -14.5, 0.5)
snr_values = np.append(snr_values, -14.75)
...
subprocess.run(
    f"cmake -DCMAKE_CXX_COMPILER=/usr/bin/g++-12 "
    f"-DCMAKE_C_COMPILER=/usr/bin/gcc-12 "
    f"-DCMAKE_BUILD_TYPE=Release -S . -B {build_dir}",
    shell=True,
    check=True,
)
