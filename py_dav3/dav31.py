...
snr_values = np.arange(-20, -16.5, 0.5)
snr_values = np.append(snr_values, -16.75)
...
subprocess.run(
    f"cmake -S . -B {build_dir}  -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON",
    shell=True,
    check=True,
)
