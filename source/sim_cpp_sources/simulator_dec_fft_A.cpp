...
int main(int argc, char *argv[]) {
  ...
  if (argc < 6 || argc > 7) {
    cout << "validate: NbMonteCarlo, SNR, q, N, K, (optionally N shortened)"
         << std::endl;
    return 1;
  }
  uint16_t q, N, K, n, p, NN, frozen_val = 0;
  softdata_t offset;
  ...
  for (int i = 0; i < N; i++)
    reliab_sequence_LH[i] = dec_param.reliab_sequence[N - i - 1];
  Shortened_seq = shortened_sequence(reliab_sequence_LH, N);
  uint16_t Froz_cnt = N - K, short_cnt = N - NN;
  ...
  for (int i = 0; i < FS_p1.size(); i += 1)
    frozen_symbols[FS_p1[i]] = true;

  vector<uint16_t> KSYMB_Idx;
  KSYMB_Idx.reserve(K);
  for (int i = 0; i < N; i++) {
    if (!frozen_symbols[i])
      KSYMB_Idx.push_back(i);
  }
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#pragma omp parallel
  {
    ...
    while (true) {
      ...
      EncodeChanBPSK_BinCCSK(gen, dec_param_local, table, EbN0,
                             CCSK_rotated_codes, L[0], KSYMB, KSYMB_Idx,
                             bin_mod_dict);
      ...
      for (int i = 0; i < short_pos.size(); i++) {
        llrs_n[short_pos[i]].value[0] = 1 - 1e-12;
        for (int j = 1; j < _GF_; j++)
          llrs_n[short_pos[i]].value[j] = 1e-12;
      }
      ...
      for (int i = 0; i < K; i++)
        info_sec_rec[i] = decoded_n[KSYMB_Idx[i]];
